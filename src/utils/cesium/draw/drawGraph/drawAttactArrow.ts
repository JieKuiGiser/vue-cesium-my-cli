...
import {
    CallbackProperty, Cartesian3, Color, PolygonGraphics, PolygonHierarchy, PolylineDashMaterialProperty,PolylineOutlineMaterialProperty, ScreenSpaceEventHandler, ScreenSpaceEventType,
} from 'cesium'
...
export default class DrawGraphAttactArrow extends DrawGraphBase {
    ...
    startDraw() {
        ...
        this.drawHandler?.setInputAction((movement: any) => {
            ...
            this.tooltip.showAt(movement.endPosition, '新增控制点,右键结束绘制')
    ...
    showRegion2Map(isModify?: boolean) {
        ...
        if (!this.drawConfig.material)
            this.drawConfig.material = Color.fromCssColorString('#fff').withAlpha(0.0)

        if (!this.drawConfig.outlineMaterial) {
            this.drawConfig.outlineMaterial = new PolylineOutlineMaterialProperty({
                // dashLength: 16,
                color: Color.fromCssColorString('#f00').withAlpha(0.7),
                outlineColor:Color.fromCssColorString('#f00').withAlpha(0.7),
            })
        }
